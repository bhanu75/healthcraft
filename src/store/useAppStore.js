  ...
  persist(
    (set, get) => ({
      ...
      addWeightEntry: (date, weight) => {
        ...
        if (!get().startingWeight && newData.length) {
      ...
      deleteWeightEntry: (date) =>
        set({
          weightData: get().weightData.filter((e) => e.date !== date),
        }),
      ...
      addPsychologyEntry: (entry) =>
        set({
          psychologyData: [...get().psychologyData, entry],
        }),
      ...
      updateStreak: () => {
        ...
        if (diff === 1) {
          const next = get().streakData.currentStreak + 1;
          set({
            streakData: {
              currentStreak: next,
      ...
      getStartWeight: () =>
        get().startingWeight ||
        (get().weightData.length ? get().weightData[0].weight : 0),

      getTotalChange: () =>
        (get().getCurrentWeight() - get().getStartWeight()).toFixed(1),
    }),
    {
